...
const longestString = (arr) => {
  ...
  return arr.reduce((acc, current) => {
    if (arr[acc].length >= current.length) {
      return acc;
    } else {
...
const firstLetters = (arr) => {
  return arr.map(value=> value.substr(0,1));
};
...
const onlyOddChars = (str) => {
  ...
  let newArr = arr.filter(value=>{
    if (String(str.indexOf(value)) % 2 === 1) {
      return value;
    }
  });
  return newArr.join('');
...
const allHappy = (arr) => {
  ...
  let chick = arr.filter(value=> {
    if (value.includes(':)')) {
      return value;
    }
  });
  console.log(chick);
  if (chick.length === arr.length) {
    return true;
  } else {return false;}
