...
const addValues = (arr) => {
 const sum = arr.reduce((acc, cuNumber) => {
  acc +=  cuNumber
    return acc ;
  },0)
  return sum;
...
const countNumberOfElements = (arr) => {
  const length = arr.reduce((acc, cuNumber) => {
    acc +=  1
      return acc;
    },0)
    return length;
...
const returnNames = (arr) => {
  const name = arr.reduce((acc, cuNumber) => {
    acc.push(cuNumber.name);
      return acc;
    },[])
    return name;
};
...
const reversedString = (str) => {
  ...
  const revers = str.reduce((acc, cuNumber) => {
    return cuNumber + acc;
    },'')
    return revers;
};
